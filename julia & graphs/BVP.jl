...
# ╔═╡ f7ee1000-819b-11ef-020c-29a5c8869847
using BoundaryValueDiffEq, Plots, LinearAlgebra

# ╔═╡ adb70774-6c62-4b6e-bed2-020b84e6b6e2
begin
	X = 1
	xspan = (0.0, X)
	T = 273.15 + 20
...
# ╔═╡ 1581a340-e26b-401f-a015-b74b9a9200c8
begin
	u_start = 10.0
	u_end = 5.0
	ρ_start = .198
	pressure(T,ρ) = ρ*R*T + 1
	function bc1!(residual, s, p, x)
	    residual[1] = s[1][1] - u_start
		residual[2] = s[end][1] - u_end
		residual[3] = s[1][3] - ρ_start
		#print(residual[1])
	end

	#=
	function bc2a!(resid_a, u_a, p)
    resid_a[1] = u_a[1] - u_start
	resid_a[2] = u_a[3] - p(T,ρ_start) # Pressure at start is defined.
end
function bc2b!(resid_b, u_b, p) 
    resid_b[1] = u_b[1] -  u_end # Velocity at end of pipe is u_end
end
	=#
end

# ╔═╡ 29fa04e8-3766-4bb0-8e61-e00154d56755
begin
	initialguess = [10.0,4000.0, ρ_start]
	bvp1 = BVProblem(pipe!, bc1!, initialguess, xspan)
	#bvp2 = TwoPointBVProblem(pipe!, (bc2a!, bc2b!), initialguess, xspan;
    #bcresid_prototype = (zeros(2), zeros(1)))
	#sol1 = solve(bvp1, MIRK4(), dt = 0.05)
	#plot(sol1)
end

# ╔═╡ f02aaa32-661b-4a04-96fa-91ca6ee45a67
sol1 = solve(bvp1, MIRK4(), dt = 0.05)

# ╔═╡ 160153f3-ca1c-4067-b66f-7bef2014953b
sol1[end]

# ╔═╡ 0c917f6b-0319-4e47-9751-d7be0fd272a2
sol1

# ╔═╡ 2813af47-679e-41bf-bb50-ddffaf116410
let
	X = 1
	xspan = (0.0, X)
	T = 273.15 + 20.0
...
# ╔═╡ 4c345ccd-c35b-48d2-bf4f-a068f6ff9bfc
[R*T/30,.3, ρ_start]

# ╔═╡ 99a12bdf-0271-4c9f-81ae-6157d3283819
ρ_start

# ╔═╡ c5fca7f9-38dc-441a-932e-a4fa00aad929
dx

# ╔═╡ 0d4052e8-a6f8-47eb-b384-441598becd40
1+1

# ╔═╡ da7946b7-d5fa-407a-aad1-efa33e289a7a
...
manifest_format = "2.0"
project_hash = "c8d569be73f87ec525fc230b2e41f70c593aea47"
