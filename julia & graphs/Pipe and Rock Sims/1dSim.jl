...
# ╔═╡ 6fe665e4-f3c0-11ef-0ae7-23ed32fbe63f
using DifferentialEquations, Plots


# ╔═╡ 896fdd29-e03b-4cf6-a6f2-edd5e3452ecc
begin
	#OLD FORMULATION
	
	# Define parameters
	u0 = 1.0           # Initial velocity
	mach = 0.2         # Initial Mach number
	reynolds = 5.0    # Initial Reynolds number
	
	# Compute r_+ and r_-
	discriminant = 1 - 4 * mach^(-2) / (mach^(-2) + 1 - reynolds^(-1))^2
	
	if discriminant < 0
	    error("No real roots for r_+ and r_-. Adjust parameters!")
	end
	
	r_plus = (mach^(-2) + 1 - reynolds^(-1)) / 2 * (1 + sqrt(discriminant))
	r_minus = (mach^(-2) + 1 - reynolds^(-1)) / 2 * (1 - sqrt(discriminant))
	
	# Define the ODE for du/dx
	function du_dx!(du, x, params, u)
	    u0, r_plus, r_minus, reynolds = params
	    if 1/(abs(u/u0 - r_plus)) < 1e-6 || 1/(abs(u/u0 - r_minus)) < 1e-6  # Avoid singularities
	        du[1] = 0.0  # Prevent division by zero
	    else
	        du[1] = 1/((u/u0) / ((u/u0 - r_plus) * (u/u0 - r_minus)))
	    end
	end
	
	# Define the x-domain (independent variable range)
	x_span = (.000001, 30.0)  # Solve from x = 0 to x = 10
	
	# Pack parameters
	params = (u0, r_plus, r_minus, reynolds)
	
	# Define the ODE problem correctly
	prob = ODEProblem(du_dx!, [u0], x_span, params)
	
	# Solve ODE
	sol = solve(prob, Tsit5(), reltol=1e-8, abstol=1e-8)
end

# ╔═╡ f59dfa8a-88e7-4561-8468-313b864372a3
r_plus

# ╔═╡ 33edbf71-8a22-4caf-8729-b3f17e0001e7
r_minus

# ╔═╡ 71098a8f-4938-4f2c-84f9-db6da312feff
#sol

# ╔═╡ 9e15443f-949d-4bf3-8d3c-ff02ba551708
begin
	x_vals = sol.t
	u_vals = [el[1] for el in sol.u]
	
	# Plot numerical solution correctly
	plot(u_vals, x_vals, label="Numerical Solution", xlabel="u", ylabel="x", linewidth=2, title = "u(x) for u0 = $(u0), Mach = $(mach), Reynolds = $(reynolds), \nr+ = $(round(r_plus, digits=3)), r-=$(round(r_minus, digits=3))\n ", yflip=true, legend=false)

	hline!([r_plus, r_minus])


	#print("u0 = $(u0), Mach = $(mach), Reynolds = $(reynolds), \nr+ = $(r_plus)\nr-=$(r_minus) ")
	
end

# ╔═╡ 39af4b5c-b0e6-4609-9714-fdf936d3eb04
x_vals

# ╔═╡ 6f903447-3896-4c1d-ab4f-9be650ff163c
begin
	# Define analytical solution
	function x_analytical(u, u0, re, r_plus, r_minus)
	    return (1 / (re * u0)) * (u0 / (r_plus - r_minus)) * 
	           (r_plus * log(abs.((r_plus .- u/u0) ./ (r_plus - 1))) -
	            r_minus * log(abs.((r_minus .- u/u0) ./ (r_minus - 1))))
	end
	
	# Compute analytical solution for comparison
	u_analytical = range(.4, 30, length=1000)
	x_analytical_vals = x_analytical.(u_analytical, u0, reynolds, r_plus, r_minus)
	
	# Overlay analytical solution
	plot!(x_analytical_vals, u_analytical, label="Analytical Solution", linestyle=:dash)
	
	#title!("Verification of Steady-State 1D Flow")
	display(plot!())
end

# ╔═╡ fdbb066c-109f-4865-a7f8-24e0911477b6
plot(u_analytical, x_analytical_vals, title = "analytical u(x) for u0 = $(u0), Mach = $(mach), Reynolds = $(reynolds), \nr+ = $(round(r_plus, digits=3)), r-=$(round(r_minus, digits=3))\n ", yflip=true, legend=false)

# ╔═╡ 5272c6d1-ea5c-407b-b51e-472c6c0122d5
x_analytical_vals

# ╔═╡ 67074ce1-808b-4788-84b3-94fc38c95947
(x,y)
...
manifest_format = "2.0"
project_hash = "2b6dc4b52f82466148c9f0313da855c0fd469fe1"
