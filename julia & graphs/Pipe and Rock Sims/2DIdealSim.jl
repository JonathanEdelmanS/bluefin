...
# ╔═╡ 7acd96cd-8937-4754-abb9-e75f3ca7e8e1
begin
	#--------------------------------------------------------------------------
...
# ╔═╡ bfa53fbb-3a51-4385-a820-0b43af288136
begin
...
# ╔═╡ e68ebcea-6994-48f1-bf76-774c71d7bc4e


# ╔═╡ 4b0d41ec-cc66-4816-a21f-c497d6c79864
prop = zeros(11, 100)

# ╔═╡ 8248a1d1-c681-442b-96a3-14760ca2fd23


# ╔═╡ 521b4bdb-9a05-4530-83a8-611e1dc7906c
function propIdealT_allYs(prop, timesteps)
    y_slices = 0:11
    ny = length(y_slices)
    nt = timesteps

    # Read one file to get number of x points
    time = lpad(1, 4, '0')
    temp_df = CSV.read("../../projects/bluefin/problems/Laforce/2D_IdealFluid_csv_horizontal_y0_$time.csv", comment="#", DataFrame)
    nx = size(temp_df, 1)

    # Preallocate list of matrices (each matrix is nx by ny)
    allgas = Vector{Matrix{Float64}}(undef, nt)

    for t in 1:nt
        time = lpad(t, 4, '0')
        mat = Matrix{Float64}(undef, nx, ny)
        for (j, y) in enumerate(y_slices)
            filename = "../../projects/bluefin/problems/Laforce/2D_IdealFluid_csv_horizontal_y$(y)_$time.csv"
            cur = CSV.read(filename, comment="#", DataFrame)
            mat[:, j] = cur[!, prop]
        end
        allgas[t] = mat
    end
...
# ╔═╡ b1d2e2f5-7631-4d11-b6dc-85478ccc048b
mats = propIdealT_allYs("sgas", 593)

# ╔═╡ 32686d88-c295-4f6f-bb63-3091db63d67f
pressures = propIdealT_allYs("pwater", 593)

# ╔═╡ 4394a484-541b-4f9e-90db-d4e95fc6a605
temps = propIdealT_allYs("temp", 593)

# ╔═╡ 062ace3e-f954-4b8b-b18b-71a29342a1ab
densities = [pressures[i] ./ (temps[i] .* m.r) for i in 1:593]

# ╔═╡ a796a7e3-920f-4444-8c28-28f9a7863a19
@bind t Slider(1:593)

# ╔═╡ c12bfb1d-d521-4cf9-a6bd-37a772bb1c16
heatmap(densities[t]', ylabel = "z (m)", xlabel = "r (m)", xlims = (1,50), title = "Density")

# ╔═╡ 617c8cc5-c241-493f-8317-399d1185dca4
heatmap(mats[t]', ylabel = "z (m)", xlabel = "r (m)", xlims = (1,50), clims=(0,.8), title = "Saturation")

# ╔═╡ b70eda61-0e6a-42b8-9eec-6e81417c80ef
scatter(mats[t]'[:,5], 1:12, ylabel = "z (m)", xlabel = "prop")

# ╔═╡ 007f673b-1222-4d08-bef4-b375034c034b
pwd()

# ╔═╡ a4322a2e-eba4-4621-83dd-4154b280bdc2
propIdealT_allYs("pwater", 2)

# ╔═╡ 3a2ec8f9-359d-4834-84b4-d8a32000e1b7
mats[t]'
...
manifest_format = "2.0"
project_hash = "5c9f58d8de2b09fb41ecc36ba0306f8ed8276b5b"
