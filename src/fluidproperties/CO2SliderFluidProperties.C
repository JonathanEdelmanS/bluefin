...
#include "CO2SliderFluidProperties.h"
#include "BrentsMethod.h"
#include "Conversion.h"
#include "MathUtils.h"
#include "libmesh/utility.h"

registerMooseObject("FluidPropertiesApp", CO2SliderFluidProperties);

InputParameters
CO2SliderFluidProperties::validParams()
{
  InputParameters params = HelmholtzFluidProperties::validParams();
  params += NaNInterface::validParams();

  params.addParam<Real>("real_coef", 1, "Realness coefficient of the CO2");
...
CO2SliderFluidProperties::CO2SliderFluidProperties(const InputParameters & parameters)
  : HelmholtzFluidProperties(parameters),
    //NaNInterface(this),
    _real_coef(getParam<Real>("real_coef"))
{
}

CO2SliderFluidProperties::~CO2SliderFluidProperties() {}

std::string
CO2SliderFluidProperties::fluidName() const
{
  return "co2";
}

Real
CO2SliderFluidProperties::molarMass() const
{
  return _Mco2;
}

Real
CO2SliderFluidProperties::criticalPressure() const
{
  return _critical_pressure;
}

Real
CO2SliderFluidProperties::criticalTemperature() const
{
  return _critical_temperature;
}

Real
CO2SliderFluidProperties::criticalDensity() const
{
  return _critical_density;
}

Real
CO2SliderFluidProperties::triplePointPressure() const
{
  return _triple_point_pressure;
}

Real
CO2SliderFluidProperties::triplePointTemperature() const
{
  return _triple_point_temperature;
}

Real
CO2SliderFluidProperties::meltingPressure(Real temperature) const
{
  if (temperature + +273.15 < _triple_point_temperature)
    throw MooseException("Temperature is below the triple point temperature in " + name() +
                         ": meltingPressure()");

  Real Tstar = (temperature+273.15) / _triple_point_temperature;
...
Real
CO2SliderFluidProperties::sublimationPressure(Real temperature) const
{
  if (temperature  +273.15> _triple_point_temperature)
...
Real
CO2SliderFluidProperties::vaporPressure(Real temperature) const
{
...
void
CO2SliderFluidProperties::vaporPressure(Real, Real &, Real &) const
{
  mooseError("vaporPressure() is not implemented");
}

Real
CO2SliderFluidProperties::saturatedLiquidDensity(Real temperature) const
{
  if (temperature < _triple_point_temperature || temperature > _critical_temperature)
...
Real
CO2SliderFluidProperties::saturatedVaporDensity(Real temperature) const
{
  if (temperature < _triple_point_temperature || temperature > _critical_temperature)
...
Real
CO2SliderFluidProperties::alpha(Real delta, Real tau) const
{
  ...
  return phi0 + phir * _real_coef;
}

Real
CO2SliderFluidProperties::dalpha_ddelta(Real delta, Real tau) const
{
  ...
  return dphi0dd + dphirdd * _real_coef;
}

Real
CO2SliderFluidProperties::dalpha_dtau(Real delta, Real tau) const
{
  ...
  return dphi0dt + dphirdt * _real_coef;
}

Real
CO2SliderFluidProperties::d2alpha_ddelta2(Real delta, Real tau) const
{
  ...
  return d2phi0dd2 + d2phirdd2 * _real_coef;
}

Real
CO2SliderFluidProperties::d2alpha_dtau2(Real delta, Real tau) const
{
  ...
  return d2phi0dt2 + d2phirdt2 * _real_coef;
}

Real
CO2SliderFluidProperties::d2alpha_ddeltatau(Real delta, Real tau) const
{
  ...
  return d2phirddt * _real_coef;
}

Real
CO2SliderFluidProperties::p_from_rho_T(Real density, Real temperature) const
{
...
Real
CO2SliderFluidProperties::rho_from_p_T(Real pressure, Real temperature) const
{
  // Check that the input parameters are within the region of validity
  ...
  if (((temperature +273.15> _triple_point_temperature) && (pressure > meltingPressure(temperature))) ||
      ((temperature +273.15< _triple_point_temperature) && (pressure > sublimationPressure(temperature))))
    throw MooseException("Input pressure and temperature in " + name() +
                         ": rho_from_p_T() correspond to solid CO2 phase" + ". T: " + std::to_string(temperature) + ", p: " + std::to_string(pressure) +
                         "triple_point_temperature: " + std::to_string(_triple_point_temperature) + ", MP: " + std::to_string(meltingPressure(temperature))
                         );
  ...
  { return p_from_rho_T(x, temperature+273.15) - pressure; };
...
void
CO2SliderFluidProperties::rho_from_p_T(
...
Real
CO2SliderFluidProperties::mu_from_p_T(Real pressure, Real temperature) const
{
...
void
CO2SliderFluidProperties::mu_from_p_T(
...
Real
CO2SliderFluidProperties::mu_from_rho_T(Real density, Real temperature) const
{
  // Check that the input parameters are within the region of validity
  //if (temperature < 216.0 || temperature > 1000.0 || density > 1400.0)
  //  throw MooseException("Parameters out of range in " + name() + ": mu_from_rho_T()" + "temperature:" + temperature + "density: " + density);
...
void
CO2SliderFluidProperties::mu_from_rho_T(Real density,
                                  Real temperature,
                                  Real ddensity_dT,
                                  Real & mu,
                                  Real & dmu_drho,
...
void
CO2SliderFluidProperties::rho_mu_from_p_T(Real pressure, Real temperature, Real & rho, Real & mu) const
{
...
void
CO2SliderFluidProperties::rho_mu_from_p_T(Real pressure,
                                    Real temperature,
                                    Real & rho,
                                    Real & drho_dp,
                                    Real & drho_dT,
...
Real
CO2SliderFluidProperties::partialDensity(Real temperature) const
{
...
std::vector<Real>
CO2SliderFluidProperties::henryCoefficients() const
{
  return {-8.55445, 4.01195, 9.52345};
}

Real
CO2SliderFluidProperties::k_from_p_T(Real pressure, Real temperature) const
{
...
void
CO2SliderFluidProperties::k_from_p_T(
...
Real
CO2SliderFluidProperties::k_from_rho_T(Real density, Real temperature) const
{
